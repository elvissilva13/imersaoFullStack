...
import (
	...
	"github.com/elvissilva13/imersaoFullStack/goapi/internal/entity"
)
...
func (pd *ProductDB) GetProducts() ([]*entity.Product, error) {
	rows, err := pd.db.Query("SELECT id,name,price,category_id From produtcs")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (pd *ProductDB) GetProduct(id string) (*entity.Product, error) {
	var product entity.Product
	err := pd.db.QueryRow("SELECT id , name ,price,category_id,image_url FROM EHERE id=?", id).
		Scan(&product.ID, &product.Name, &product.CategoryID, &product.ImageURL)
	if err != nil {
		return nil, err
	}
	return &product, nil
...
func (pd *ProductDB) GetProductByCategoryID(categoryID string) ([]*entity.Product, error) {
	rows, err := pd.db.Query("SELECT id, name, description, category_id, image_url FROM products Where category_id = ?", categoryID)
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		var product entity.Product
		if err := rows.Scan(&product.ID, &product.Name, &product.Price, &product.Description, &product.ImageURL); err != nil {
			return nil, err
		}
		products = append(products, &product)
	}
