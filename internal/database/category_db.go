...
func (cd *CategoryDB) GetCategories(id, name string) ([]*entity.Category, error) {
...
func (cd *CategoryDB) GetCategory(id string) (*entity.Category, error) {
	var category entity.Category
	err := cd.db.QueryRow("SELECT id, name FROM category WHERE id=?", id).Scan(&category.ID, &category.Name)
	if err != nil {
		return nil, err
	}
	return &category, nil
